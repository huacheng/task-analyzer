...
├── .references/               # External reference materials (by topic, collected during plan/exec)
...
`AiTasks/.references/<topic>.md` is a shared resource across tasks. When `plan` or `exec` writes to it (reference collection), it MUST acquire `AiTasks/.references/.lock` using the same lock protocol above. This prevents concurrent tasks from corrupting the reference files.
